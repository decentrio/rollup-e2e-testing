...
func (c *CosmosChain) BuildRelayerWallet(ctx context.Context, keyName string) (ibc.Wallet, error) {
	coinType, err := strconv.ParseInt(c.cfg.CoinType, 10, 32)
	if err != nil {
...
func (c *CosmosChain) SendIBCTransferAfterHardFork(
	...
	txHash, err := c.getFullNode().SendIBCTransferAfterHardFork(ctx, channelID, keyName, toWallet, options, c.getValNode().HomeDir())
	if err != nil {
		return tx, fmt.Errorf("send ibc transfer: %w", err)
	}
	txResp, err := c.GetTransaction(txHash)
