...
func (c *DymHub) StartWithoutGenesisAccount(testName string, ctx context.Context, additionalGenesisWallets ...ibc.WalletData) error {
	// Start chain
	chainCfg := c.Config()
	...
	for _, r := range rollApps {
		...
		checksum := r.GetChecksum(ctx)
		if err := c.RegisterRollAppToHubWithoutGenesisAccount(ctx, sequencerName, bech32, rollAppChainID, checksum, sequencer, r.(ibc.Chain).Config().Bech32Prefix, keyDir, flags); err != nil {
			return fmt.Errorf("failed to start chain %s: %w", c.Config().Name, err)
		}
...
// RegisterRollAppToHub register rollapp on settlement.
func (c *DymHub) RegisterRollAppToHubWithoutGenesisAccount(ctx context.Context, keyName, bech32, rollappChainID, checksum, sequencerAddr, bech32Prefix, keyDir string, flags map[string]string) error {
	return c.GetNode().RegisterRollAppToHubWithoutGenesisAccount(ctx, keyName, bech32, rollappChainID, checksum, sequencerAddr, bech32Prefix, keyDir, flags)
}
