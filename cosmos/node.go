	...
	if lastThree == "EVM" {
		...
		if tokenless {
			...
			command = append(
				command, "rollapp", "create-rollapp",
				rollappChainID, string(alias), vmtype, "--bech32-prefix", bech32Prefix, "--init-sequencer", sequencerAddr, "--genesis-checksum", checksum, "--metadata", keyDir+"/metadata.json", "--genesis-accounts", bech32+":"+dymension.GenesisEventAmount.String(),
				"--native-denom", keyDir+"/native_denom_tokenless.json", "--initial-supply", "10000200000000000000000000",
				"--broadcast-mode", "async", "--keyring-dir", keyPath)
		} else {
			vmtype = "WASM"
			command = append(
				command, "rollapp", "create-rollapp",
				rollappChainID, string(alias), vmtype, "--bech32-prefix", bech32Prefix, "--init-sequencer", sequencerAddr, "--genesis-checksum", checksum, "--metadata", keyDir+"/metadata.json", "--genesis-accounts", bech32+":"+dymension.GenesisEventAmount.String(),
				"--native-denom", keyDir+"/native_denom.json", "--initial-supply", "10000200000000000000000000",
				"--broadcast-mode", "async", "--keyring-dir", keyPath)
		}
	}
...
func (node *Node) RegisterRollAppToHubWithoutGenesisAccount(ctx context.Context, keyName, bech32, rollappChainID, checksum, sequencerAddr, bech32Prefix, keyDir string, flags map[string]string) error {
	...
	if lastThree == "EVM" {
		...
		command = append(
			...
			"--native-denom", keyDir+"/native_denom.json", "--initial-supply", "100000010000000000000000000000000",
			"--broadcast-mode", "async", "--keyring-dir", keyPath)
	} else {
		vmtype = "WASM"
		command = append(
...
func (node *Node) WasmInstantiateContract(ctx context.Context, keyName, codeId, initCW20, label string) error {
	var command []string
	command = append(command, "wasm", "instantiate", codeId, initCW20, "--label", label, "--no-admin")
...
func (node *Node) QueryWasmContractICS20ChannelsSmart(ctx context.Context, keyName, ics20Addr, queryMsg string) (QuerySmartContractICS20ChannelsResponse, error) {
	command := []string{"wasm", "contract-state", "smart", ics20Addr, queryMsg}
	stdout, _, err := node.ExecQuery(ctx, command...)
	if err != nil {
		return QuerySmartContractICS20ChannelsResponse{}, err
	}

	var resp QuerySmartContractICS20ChannelsResponse
	err = json.Unmarshal(stdout, &resp)
	if err != nil {
		return QuerySmartContractICS20ChannelsResponse{}, err
	}
...
func (node *Node) QueryWasmContractICS20PortSmart(ctx context.Context, keyName, ics20Addr, queryMsg string) (QuerySmartContractICS20PortResponse, error) {
	command := []string{"wasm", "contract-state", "smart", ics20Addr, queryMsg}
	stdout, _, err := node.ExecQuery(ctx, command...)
	if err != nil {
		return QuerySmartContractICS20PortResponse{}, err
	}

	var resp QuerySmartContractICS20PortResponse
	err = json.Unmarshal(stdout, &resp)
	if err != nil {
		return QuerySmartContractICS20PortResponse{}, err
	}
