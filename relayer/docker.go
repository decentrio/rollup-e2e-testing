...
func (r *DockerRelayer) AddChainConfiguration(ctx context.Context, rep ibc.RelayerExecReporter, chainConfig ibc.ChainConfig, keyName, rpcAddr, grpcAddr, apiAddr string, trusting_period int64) error {
	...
	configContent, err := r.c.ConfigContent(ctx, chainConfig, keyName, rpcAddr, grpcAddr, apiAddr, trusting_period)
	if err != nil {
...
type RelayerCommander interface {
	...
	ConfigContent(ctx context.Context, cfg ibc.ChainConfig, keyName, rpcAddr, grpcAddr, apiAddr string, trusting_period int64) ([]byte, error)
